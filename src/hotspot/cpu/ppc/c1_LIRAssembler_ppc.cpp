...
void LIR_Assembler::emit_profile_multiple_array_types(LIR_OpProfileMultipleArrayTypes* op) {
  ...
  __ increment_mem64(mdo, md->byte_offset_of_slot(op->load(), ArrayLoadData::not_flat_nullable_count_offset()),
                     DataLayout::counter_increment, tmp1);
  __ b(done);

  __ bind(null_free);

  __ increment_mem64(mdo, md->byte_offset_of_slot(op->load(), ArrayLoadData::not_flat_null_free_count_offset()),
                     DataLayout::counter_increment, tmp1);
